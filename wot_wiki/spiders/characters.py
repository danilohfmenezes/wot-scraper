...
class CharactersSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for letter in alphabet:
            ...
            # yield response.follow(url=relative_url, callback=self.parse_letter, meta={'letter': letter})
            relative_url = f'/wiki/Category:Wolves?from={letter}'
            yield response.follow(url=relative_url, callback=self.parse_letter, meta={'letter': letter})
            relative_url = f'/wiki/Category:Horses?from={letter}'
            yield response.follow(url=relative_url, callback=self.parse_letter, meta={'letter': letter})

        missing_characters = [
            '/wiki/Jain_Farstriders',
            '/wiki/Narg'
        ]
        for character_url in missing_characters:
    ...
    def parse_character(self, response):
        ...
        yield {
            ...
            'animal_type': self.get_attribute(response, 'animal'),
            'color': self.get_attribute(response, 'color'),
            'owner': self.get_attribute(response, 'owner'),
        }
